...
**Reads:** `.blog/registry.json` · `.blog/clients/<client>/brand.md` · an optional `parsed_content_packet` from `cursive-agent-2-content-parse`
...
**1. A `parsed_content_packet`, if one exists.** `cursive-agent-2-content-parse` already parsed the live site for content inventory, brand signals, tone, sentiment, offers, proof points, and gaps. Ingest it rather than re-deriving — re-deriving costs a crawl and produces a second, differently-worded answer to a question already answered, after which two files disagree and nobody knows which is current. Ask the user for the packet by path, pasted JSON, or Sheet row. If they don't have one and the site is live, offer to run `cursive-agent-2-content-parse` first; don't crawl the site yourself.

Map it like this:

| Packet field | brand.md section |
|---|---|
| offers, services | What they sell |
| audience signals | Who buys it |
| proof points | Proof points on record |
| tone, sentiment, quoted lines | Tone: how they talk |
| named gaps | stay gaps — carry them into Phase 3 |

The one trap: a packet's "differentiators" are usually the homepage's adjectives — *trusted*, *premium*, *results-driven*. Those are evidence of how the client talks, so they belong under **Tone**. They are not proof, so they do not belong under **What makes them different**. Filing them in the wrong section is how a profile becomes confidently wrong.
...
Set `source` in the frontmatter accordingly: `parsed_content_packet` if the profile came from a packet alone, `interview` if it came from the conversation alone, `both` when a packet was ingested and then confirmed or extended in Phase 3.
