# Opinion Bank

The author's accumulated record of positions, stories, specifics, and voice. Loaded at the start of every interview so answered questions are never re-asked; updated after every interview with anything durable.

Rules for maintaining this file:

- Date every entry (YYYY-MM).
- Store durable material only — standing positions and reusable stories, not one-off post details.
- Numbers do not live here. Prices, timelines, thresholds, and measurable results go to `facts.json`, where they carry a verification date and an expiry. A number in prose can't be checked for staleness; a number in the vault can.
- Keep the author's actual wording. This file is a voice archive, not a summary.
- When a position changes, don't delete the old one — mark it superseded with the date. Changed minds are great article material.

---

## Author profile

<!-- Fill in during the first interview. -->
- **Name:**
- **Business:**
- **What they do, in their own words:**
- **Who their customers are:**
- **Years in the field / background:**

## Standing positions

<!-- The author's core beliefs about their field. Format:
### [Topic] (YYYY-MM)
Position in the author's own words. -->

## Contrarian takes

<!-- Things the author believes that the standard advice or competitors would dispute. -->

## War stories and first-hand evidence

<!-- Real client stories, projects, mistakes, results. These are the ONLY experiences
that may ever be presented as first-hand in published content. Format:
### [Short story title] (YYYY-MM)
What happened, with the specifics. Note any anonymization the author requested. -->

## Voice fingerprints

<!-- Exact phrases, analogies, and expressions the author uses. Quote, don't paraphrase. -->

## Boundaries — never publish

<!-- Claims, promises, names, and framings the author has ruled out. Check this list
during drafting AND final review of every post. -->

## Topic log

<!-- One line per interview: date, topic, packet filename. Prevents re-interviewing
covered ground and shows what the bank already knows. -->
